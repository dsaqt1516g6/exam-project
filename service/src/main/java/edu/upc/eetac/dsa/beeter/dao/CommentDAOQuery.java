...
public class CommentDAOQuery
{
    ...
    public final static String GET_COMMENTS = "select hex(s.id) as id, hex(s.user_id) as user_id, s.text, hex(s.exam_id) as exam_id, s.created_at from comment_exam s, users u where created_at < ? and u.id=s.user_id order by created_at desc limit 25";
