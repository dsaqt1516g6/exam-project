...
public class ExamDAOQuery
{
    ...
    public final static String GET_EXAMS = "select hex(s.id) as id, hex(s.user_id) as user_id, s.text, s.image, s.created_at, subject from exam s, users u where created_at < ? and u.id=s.user_id order by created_at desc limit 25";
    public final static String GET_EXAMS_AFTER = "select hex(s.id) as id, hex(s.user_id) as user_id, s.text, s.image, s.created_at, subject from exam s, users u where created_at > ? and u.id=s.user_id order by created_a desc limit 25";
